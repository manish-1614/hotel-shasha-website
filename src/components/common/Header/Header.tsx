  ...
  useEffect(() => {
    if (isMobileMenuOpen) {
      ...
      const hamburgerButton =
        hamburgerContainerRef.current?.querySelector('button');
  ...
  return (
    <>
      ...
      <ClientOnly
        fallback={
          <header
            className={`${styles.headerContainer} transition-all duration-300 bg-white/95 backdrop-blur-sm shadow-sm ${className}`}
          >
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className={styles.headerContent}>
              ...
              <button
                type="button"
                onClick={() => handleNavClick('home')}
                className="flex items-center group p-2 -m-2 touch-manipulation min-h-[44px] min-w-[44px] transition-transform duration-300 hover:scale-125 hover:rotate-[-15deg]"
                aria-label="Go to home section"
                title="Hotel Shasha - Go to home"
              >
                <Image
                ...
                >
                  ...
                  >
                    <div id="mobile-menu-description" className="sr-only">
                      Use arrow keys to navigate between menu items, Enter to
                      select, or Escape to close the menu.
                    </div>
                    <motion.div
                      className="space-y-2"
                    ...
                    >
                      {navigationItems.map((item, index) => (
                        ...
                        >
                          ...
                          <span
                            id={`nav-item-${item.href}-description`}
                            className="sr-only"
                          >
                    ...
                    >
                      <div className="text-center space-y-3">
                        <p className="text-lg font-medium text-mountain-700">
                          Experience the serenity of Jibhi
                        </p>
                        <div className="space-y-1">
                          <p className="text-base text-mountain-600">
                            +91 98765 43210
                          </p>
                          <p className="text-base text-mountain-600">
                            info@hotelshasha.com
                          </p>
                        </div>
                      </div>
                    </motion.div>
